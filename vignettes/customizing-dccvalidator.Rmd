...
* `templates` (including `manifest_template`, `individual_templates`,
  ...
  Synapse-registered JSON schema. For templates accessed via synID, these should
  be either .xlsx or .csv files, where the column names reflect the required
  columns in the template. If the template is an excel file with multiple
  sheets, the first sheet will be used. For templates accessed via JSON schema
  id, these should have a `properties` object listing all column names (a.k.a.
  keys) that should be present in the metadata. The `manifest_template` should
  have only one template. `individual_templates` and `biospecimen_templates`
  should be broken up by species (the species must be included in the
  `species_list`). Additionally, the `biospecimen_templates` may also be divided
  by specimen type (e.g. in vitro, postmortmem, other). Note that this requires
  `include_biospecimen_type` to be `TRUE`). The types specified in the config
  will be reflected in the application UI. The `assay_templates` should be
  broken up by `assay` (the list of assay templates available will be reflected
