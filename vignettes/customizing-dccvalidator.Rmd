...
1. (For running locally only): create a .synapseConfig file in your home directory that has your Synapse authentication credentials (see the [synapser docs for an example](https://r-docs.synapse.org/articles/manageSynapseCredentials.html#use-synapseconfig)).
...
* `app_url`: The URL for the app when in production. This is required for using the Synapse OAuth 2.0 client for logging into the application. See [Deploying dccvalidator](https://sage-bionetworks.github.io/dccvalidator/articles/deploying-dccvalidator.html) for more information.
