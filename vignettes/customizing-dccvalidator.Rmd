...
* `templates` (including `manifest_template`, `individual_templates`,
  ...
  use for validation. These may be a Synapse synID or the id of a Synapse-registered JSON schema. For templates accessed via synID, these should be either .xlsx or .csv files, where the
  column names reflect the required columns in the template. If the template is
  an excel file with multiple sheets, the first sheet will be used. For templates accessed via JSON schema id, these should have a `properties` object listing all column names (a.k.a. keys) that should be present in the metadata.
