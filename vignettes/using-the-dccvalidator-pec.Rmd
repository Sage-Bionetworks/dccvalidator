...
Each study in PEC will have accompanying documentation in the PEC portal. Here is an example of study [documentation in the Accelerating Medicines Partnership in Alzheimer's Disease portal](https://adknowledgeportal.synapse.org/#/Explore/Studies?Study=syn3219045), developed by Sage Bionetworks. 
...
Once data has passed validation and the PEC data curators permit edit permissions to the **Staging** folder, data must be uploaded using the manifest. Upload using a manifest is achievable with the [R, Python and command line clients](https://docs.synapse.org/articles/uploading_in_bulk.html).
...
Please send questions and requests for new keys or values to [PEC_SageAdmin@synapse.org](mailto:PEC_SageAdmin@synapse.org). 
