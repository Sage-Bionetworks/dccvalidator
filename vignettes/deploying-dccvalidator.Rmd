...
4. If needed, make any changes to the application's behavior by editing the
...
6. The Sage Shiny Server administrator must configure the utilization scheduler to ensure each connection has its own R process. 
```
location /users {
  location /yourusername/ShinyApps {
