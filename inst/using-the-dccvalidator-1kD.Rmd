...
Each study should have accompanying documentation in the 1kD Synapse project. 
...
Each study should include metadata that would help a new researcher understand and reuse the data. In most cases, we will expect 2 files:
  
1. A **[1kD_minimum_ChildParent_metadata](https://www.synapse.org/#!Synapse:syn29358381)** describing each child/parent/other caregiver in the study
- Each row corresponds to a unique individual
2. A **[AssayManifest](https://www.synapse.org/#!Synapse:syn27832372)** listing each file and its assay specific information that will be uploaded. Remember to include your metadata files in the manifest.
2. A **[Biospecimen metadata]** describing the specimens that were collected. *Only applicable to metabolomics and metagenomics data.* 
...
- A metadata template is used
- All required columns from the templates are present
- Individuals and specimens have unique identifiers, and the identifiers match across different metadata files
- Metadata terms conform to a controlled vocabulary, where applicable
- Duplicates file path in the manifest
